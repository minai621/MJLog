...
```mermaid
sequenceDiagram
    participant MacBook as 맥북
    participant GitHub as GitHub
    participant Actions as GitHub Actions
    participant Webhook as Webhook 서버 (포트 3100)
    participant Website as mj-dev.site (Next.js, 포트 3000)

    MacBook->>GitHub: 블로그 게시글 작성 및 Push
    GitHub->>Actions: 트리거 GitHub Actions
    Actions->>Webhook: curl 요청 (mj-dev.site/webhook/pull)
    Webhook->>Webhook: webhook 수신
    Webhook->>Website: deploy.sh 실행
    Website->>Website: Next.js 서버 재실행 (포트 3000)
```
