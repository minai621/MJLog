---
title: '웹뷰 React Injection으로 성능 최적화하기'
date: '2024-07-17'
tags: ['react', 'react-native', 'webview', 'performance', 'optimization']
draft: false
summary: '리액트 네이티브 앱에서 웹뷰 성능을 최적화하기 위해 리액트를 주입하는 방법과 그 효과를 분석합니다.'
---
...
![웹뷰 in 리액트](/static/images/webview-inject/webview-in-react.png)
...
<img src="/static/images/webview-inject/buttons.png" width="200" height="400" />
...
![번들 차이](/static/images/webview-inject/diff-bundle.png)

이제는 빌드된 js에는 `ReactDOM.createRoot`으로부터 생성되는 리액트 트리를 생성하는 코드만 남고, 리액트 모듈은 번들링되지 않아요.

![번들 크기](/static/images/webview-inject/size-bundle.png)
...
<WebView
  ...
  onLoadEnd={() => {
    if (webViewRef.current) {
      webViewRef.current.injectJavaScript(reactContent + '\n' + reactDOMContent)
    }
  }}
/>
```
...
![LTE환경 이미지](/static/images/webview-inject/image.png)
...
![1Mbps환경 이미지](/static/images/webview-inject/image-1.png)
...
![400kbps환경 이미지](/static/images/webview-inject/image-2.png)
...
![시뮬레이터](https://github.com/minai621/webview-inject-test/raw/main/assets/simulator.gif)
